...
X_test = processed_data["X_test"][features]
...
for model_type in model_types:
    ...
    with open(rf'...{model_type}_final.pkl', 'wb') as f:
        pickle.dump(model, f)
    
    run["model"].upload(rf'...Saved models\{model_type}_final.pkl')

    log_metrics_and_plots(run, model_type, model, X_test=X_test, y_test=y_test, testing = True, n_bootstraps=2000)
